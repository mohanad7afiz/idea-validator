...
@pytest.fixture
def built_workbook(tmp_path: Path):
...
def test_workbook_has_one_sheet_with_correct_headers(built_workbook):
    assert len(built_workbook.sheetnames) == 1
    sheet = built_workbook[built_workbook.sheetnames[0]]
    header_row = [cell.value for cell in sheet[1]]
    assert header_row == EXPECTED_COLUMNS


def test_workbook_has_three_data_rows(built_workbook):
    sheet = built_workbook[built_workbook.sheetnames[0]]
